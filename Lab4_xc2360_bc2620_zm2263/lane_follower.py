...
def run(camera, transform, tgthsv, signhsv):
    while True:
        camera.capture('homography_calibration.jpg')
        time.sleep(2)
        im_source = cv2.imread('homography_calibration.jpg')
        ...
        cv2.imshow('calculated_hough_lines.jpg', edges)
        cv2.waitKey(200)
        ...
        #   Face forward
        if lines is None:
            relocate(camera, transform, tgthsv)
        else:
            line = lines[0][0]
            theta = line[1]
            rho = line[0]
...
def relocate(camera, transform, tgthsv):
    print "no lines found, relocate ..."
    flag = False
    deg = [15, 20, 25, 30]
    for j in range(len(deg)):
        i = deg[j]
        left_deg(i)
        print "left turn" + str(i)
        time.sleep(.5)
        flag = detect_lines(camera, transform, tgthsv)
        if flag is True:
            print "found"
            return
        right_deg(2 * i)
        print "right turn" + str(i)
        time.sleep(.5)
        flag = detect_lines(camera, transform, tgthsv)
        if flag is True:
            print "found"
...
def detect_lines(camera, transform, tgthsv):
    im = camera.capture("line.png")
    time.sleep(2)
    im = cv2.imread("line.png")
    im_roadmap = cv2.warpPerspective(im, transform, (320, 240))
    im_roadmap = cv2.GaussianBlur(im_roadmap, (5, 5), 0)
    hsv = cv2.cvtColor(im_roadmap, cv2.COLOR_BGR2HSV)
    mask = color_filter(hsv, tgthsv)
    mask = cv2.GaussianBlur(mask, (3, 3), 0)
    edges = cv2.Canny(mask, 50, 150)
    lines = cv2.HoughLines(edges, 1, np.pi / 180, 50)
...
def centralize(dist, angle):
    ...
    if angle > np.pi / 2:
        ...
        right_deg(90)
        time.sleep(1)
        print "fwd" + str(abs(160 - abs(dist)))
        fwd_cm(abs(160 - abs(dist)))
        time.sleep(1)
...
def init(camera):
    ...
    time.sleep(2)
    im_source = cv2.imread('original_image.jpg')
