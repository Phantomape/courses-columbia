...
if size(matchedPoints1,1) ~= size(matchedPoints2,1)
    error(message('vision:showMatchedPoints:numPtsMustMatch'));
end
...
if isempty(style)
   % permit empty marker style, which amounts to marker not being displayed 
    ...
    try
        ...
        error(message('vision:showMatchedPoints:invalidMarkerStyle',id));
    end
end
...
if isempty(style)
    ...
    try
        ...
        error(message('vision:showMatchedPoints:invalidLineStyle'));
    end
end
